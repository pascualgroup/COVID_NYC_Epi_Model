---
title: "Implementation of SEPIR Model"
...
## Declare model name
```{r}
full_model_name = "NYC_Covid_Model_Hyrbid_Model_1_Pre-Symp_Compartment_Set_b_a_to_0"
model_name = "N_13"
rda_index = 0
rds_index = 0
...
p = ggplot(data = comp_data_melt) +
  ...
  ylab("S over N")
p
png("../Figures/MIF_local_test_results/N_13_Model_sim_test_params_S_over_N.png")
print(p)
dev.off()
```
...
#-634.48228162    0.02216171
# This is not higher than the  true sim mean of    -634.59484667 (se 0.02017609 ), so did a longer run.
```
...
#-632.68104765  0.01661451
# This is higher than true sim mean of  -634.59484667 (se 0.02017609), so it seems to have worked (at least, this is enough to move on to a grid search using Midway).
```

# MIF Run 1
## Code to generate parameter grid for initial grid search (SEPIR Model)
```{r, cache=FALSE}
knitr::read_chunk('generate_param_grid_covid_NYC_N_13.R')
```

```{r generate_param_grid_N_13, eval = FALSE, echo = TRUE}
```

## Code for first MIF run from grid search (SEPIR Model)
```{r, cache=FALSE}
knitr::read_chunk('MIF_run_Model_N_13.R')
```

```{r mif_code_N13, eval = FALSE, echo = TRUE}
```

### Script to execute code for first MIF run on Midway computing cluster

```{r engine='bash', comment=''}
cat Midway_script_Model_N_13_Grid_Search_MIF_run_1_Profile.sbatch
```
...
Once all of the 500 array jobs submitted to Midway have finished running on the cluster, the output from each of those 250 jobs is combined into one data frame  with combinations and likelihoods.
...
png("../Figures/Profiles/N_13_Model/MIF_Run_1/MIF_Run_1_Histogram_Result_Plot.png")
...
p
png("../Figures/Profiles/N_13_Model/Anitbody_data_with_RS_calc_CI_dates.png")
print(p)
dev.off()
...
p
png("../Figures/Profiles/N_13_Model/Anitbody_data_with_RS_calc_CI_days.png")
print(p)
dev.off()
```
...
write.csv(MLE, file = "../Generated_Data/Profiles/N_13_Model/Grid_Search_MIF_run_1/MLE_Grid_Search_MIF_run_1_with_LL.csv",
          row.names = FALSE)

write.csv(MIF_run_1_MLE_params, file = "../Generated_Data/Profiles/N_13_Model/Grid_Search_MIF_run_1/MLE_Grid_Search_MIF_run_1.csv",
          row.names = FALSE)
...
write.csv(small_p_S_param, file = "../Generated_Data/Profiles/N_13_Model/Grid_Search_MIF_run_1/small_p_S_top_2_LL_comb_with_loglik.csv",
          row.names = FALSE)
...
p = ggplot(data = comp_data_melt) +
  ...
  ylab("S over N")
p
png("../Figures/Profiles/N_13_Model/MLE_S_over_N.png")
print(p)
dev.off()
```
...
p = ggplot(data = select_trajectories,
  ...
  sim_traj_scale
p
png("../Figures/Profiles/N_13_Model/MIF_Run_1/Specific_traj_S_over_N_MLE.png")
print(p)
dev.off()
```
...
p = ggplot(data = comp_data_melt) +
  ...
  ylab("R over N")
p
png("../Figures/Profiles/N_13_Model/MLE_param_R_over_N.png")
print(p)
dev.off()
```
...
p = ggplot(data = select_trajectories,
  ...
  sim_traj_scale
p
png("../Figures/Profiles/N_13_Model/MIF_Run_1/Specific_traj_R_over_N_MLE_comb.png")
print(p)
dev.off()
```
...
p = ggplot() +
  ...
  median_legend_lab + rahul_man_figure_theme +
   xlab("Days since March 1, 2020")+
  ylab("R over N") +
  geom_ribbon(data = nyc_antibdoy_df,aes(x = times, ymin = lower_CI,
                                ymax = upper_CI),
                                fill = 'grey70',
                                alpha = 0.7) +
  geom_point(data = nyc_antibdoy_df,
           aes(x = times, y = Prop_Positive), color = 'blue') +
  geom_line(data = nyc_antibdoy_df,
           aes(x = times, y = Prop_Positive), color = 'blue')
  
p
png("../Figures/Profiles/N_13_Model/MLE_param_R_over_N_vs_anitbody_test.png")
print(p)
dev.off()
```
...
png(paste0("../Figures/Profiles/", model_name, "_Model/top_2_LL_sim_plots/",
           "b_p_vs_Antibody_LL_", model_name,
           "_model_antibody_LL_from_top_2_cases_LL.png"))
print(p)
dev.off()
...
png(paste0("../Figures/Profiles/", model_name, "_Model/top_2_LL_sim_plots/",
           "b_p_vs_herd_immunity_", model_name,
           "_model_antibody_LL_from_top_2_cases_LL.png"))
print(p)
dev.off()
...
```
# Isloate top 2LL of antibodyLL from top 2 LL of Model N_13 Initial Fit
```{r}
...
top_20_LL_box = top_20_LL_end_params %>%
  filter(loglist.se < 2) %>%
  sapply(range)
write.csv(top_20_LL_box, file = "../Generated_Data/Profile_Combination_Lists/N_13_Model/orignal_20_LL_param_box_from_1st_MIF_run.csv",
          row.names = FALSE)
```
...
combine_profile_output(profile_var = "b_p", model_name = model_name)
...
# Profile over b_p (SEPIAR Model)

## Code to generate profile combinations
```{r, cache=FALSE}
knitr::read_chunk('generate_profile_combinations_covid_nyc_N_13.R')
```

```{r generate_profile_combinations_N13, eval = FALSE, echo = TRUE}
```

## Code to run MIF for profile (SEPIAR Model)
```{r, cache=FALSE}
knitr::read_chunk('MIF_run_Profile_Model_N_13.R')
```

```{r mif_code_profile_N_13, eval = FALSE, echo = TRUE}
```

### Script to execute code for b_p profile MIF run on Midway computing cluster

```{r engine='bash', comment=''}
cat Midway_script_Model_N_13_b_p_Profile.sbatch
```

### Script to execute code for G_w_y_scaling profile MIF run on Midway computing cluster

```{r engine='bash', comment=''}
cat Midway_script_Model_N_13_G_w_y_scaling_Profile.sbatch
```
...
p = ggplot(data = profile_var_profile, aes_string(x = eval(profile_var), y = "loglik")) +
  geom_point() + geom_hline(yintercept = cutoff_thres_20_LL_from_ML, color = 'red') + rahul_theme
print(p)
...
## Plot b_p profile
```{r}
plot_profiles(profile_var = "b_p", model_name = model_name)
```
...
```{r}
source("Sim_G_w_y_scaling_profile_peak_Model_N_13.R")
```

### Code for simulations
```{r, cache=FALSE}
knitr::read_chunk('Sim_G_w_y_scaling_profile_peak_Model_N_13.R')
```

```{r sim_G_w_y_scaling_profile_peak_N_13, eval = FALSE, echo = TRUE}
```
...
profile_data= read.csv(file = paste0("../Generated_Data/Profiles/", model_name, "_Model/", profile_var, "_Profile/",
                          profile_var, "_", model_name, "_profile_combined_data.csv"))
profile_peak_data_b_p = profile_data %>%
  filter(loglik > max(loglik)-2)
range(profile_peak_data_b_p$R_0)
save(profile_peak_data_b_p, file = paste0("../Generated_Data/Profiles/",
     model_name,
     "_Model/b_p_Profile/top_2_LL_of_b_p_profile.RData"))
...
### b_p profile peak
```{r}
source("Sim_b_p_profile_peak_Model_N_13.R")
```

### Code for simulations
```{r, cache=FALSE}
knitr::read_chunk('Sim_b_p_profile_peak_Model_N_13.R')
```

```{r sim_b_p_profile_peak_N_13, eval = FALSE, echo = TRUE}
```

## R over N for top parameter combinations within 2LL when fit to antibody and case data (b_p Profile)
```{r}
...
```{r}
png(paste0("../Figures/Profiles/", model_name,
 "_Model/top_2_LL_sim_plots/top_2_LL_via_antibody_comp_plots/b_p_profile_Observed_Antibody_data_vs_Ribbon_Plot_R_over_N_over_time_simulation_from_b_p_profile__2_LL_antibody_from_antibody_b_p_profile_peak_params.png"))
print(p)
dev.off()
```
...
```{r}
png(paste0("../Figures/Profiles/", model_name,
 "_Model/top_2_LL_sim_plots/top_2_LL_via_antibody_comp_plots/b_p_profile_Obs_COVID_hosp_cases_vs_Ribbon_Plot_C_Q1_over_time_simulation_from_b_p_profile__2_LL_antibody_from_antibody_b_p_profile_peak_params.png"))
print(p)
dev.off()
```


## Compare to observed data (b_p Profile)
```{r}
#all_combo_melt_data = melt(all_combo_data, id.vars = c("time", "combo_num"))
...
```{r}
png(paste0("../Figures/Profiles/", model_name,
 "_Model/top_2_LL_sim_plots/top_2_LL_via_antibody_comp_plots/b_p_profile_Obs_daily_COVID_cases_vs_Ribbon_Plot_simulated_cases_over_time_simulation_from_b_p_profile_2_LL_antibody_from_antibody_profile_peak_params.png"))
print(p)
dev.off()
```
...
```{r}
png(paste0("../Figures/Profiles/", model_name,
 "_Model/top_2_LL_sim_plots/top_2_LL_via_antibody_comp_plots/b_p_profile_log_Obs_daily_COVID_cases_vs_Ribbon_Plot_simulated_cases_over_time_simulation_from_2_LL_antibody_from_antibody_profile_peak.png"))
print(p)
dev.off()
```
## Profile analysis plots (b_p profile)
```{r}
p = ggplot(data = top_2_LL_end_subset_with_antibody_LL,
           aes(x = p_S,
               y = Antibody_Mean_LL)) +
...
png(paste0("../Figures/Profiles/", model_name, "_Model/top_2_LL_sim_plots/",
           "p_S_vs_Antibody_LL_", model_name,
           "_model_antibody_LL_from_b_p_Profile_peak_LL.png"))
print(p)
dev.off()
...
antibody_top_2_LL_from_b_p_profile_top_2_LL = top_2_LL_end_subset_with_antibody_LL %>%
  filter(Antibody_Mean_LL > max(Antibody_Mean_LL)-2)
nrow(antibody_top_2_LL_from_b_p_profile_top_2_LL)
range(antibody_top_2_LL_from_b_p_profile_top_2_LL$R_0)
p = ggplot(data = antibody_top_2_LL_from_b_p_profile_top_2_LL,
           aes(x = b_a,
               y = R_0)) +
  geom_point() +
  rahul_man_figure_theme
...
png(paste0("../Figures/Profiles/", model_name, "_Model/top_2_LL_sim_plots/",
           "top_2_LL_via_antibody_comp_plots/",
           "b_a_vs_R_0_", model_name,
           "_model_top_2_antibody_LL_from_b_p_profile_peak_LL.png"))
print(p)
dev.off()
...
range(antibody_top_2_LL_from_b_p_profile_top_2_LL$b_q)
hist(antibody_top_2_LL_from_b_p_profile_top_2_LL$R_0)
```


## Calculate R_0 NGM for top_2_LL of b_p profile
...
p = ggplot(data = top_2_LL_end_subset_with_antibody_LL,
  ...
  rahul_man_figure_theme
p
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_Profile/", model_name,
                  "_b_p_profile_density_plot_of_R_0_NGM_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
...
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_Profile/", model_name,
                  "_b_p_Profile_histogram_of_R_0_NGM_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
...
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_Profile/", model_name,
                  "_b_p_Profile_R_0_A_vs_R_0_S_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
...
p
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_Profile/", model_name,
                  "_b_p_Profile_R_0_vs_R_0_NGM_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
...
p
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_Profile/", model_name,
                  "_b_p_Profile_R_0_vs_R_0_S_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
...
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_profile/", model_name,
                  "_b_p_profile_R_0_NGM_vs_Likelihood_with_respect_to_antibody_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
...
png(file = paste0("../Figures/Profiles/", model_name, "_Model/b_p_profile/", model_name,
                  "_b_p_profile_b_a_vs_R_0_NGM_for_all_parameter_combinations_within_2LL_of_profile_MLE_fit_to_cases.png"))
print(p)
dev.off()
```
